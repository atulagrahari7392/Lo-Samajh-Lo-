        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-blue-100">
              {isHindi ? 'आधिकारिक एनसीईआरटी पाठ्यपुस्तक पुस्तकालय' : 'NCERT Official Textbooks Library'}
            </span>
          </div>
          <div className="flex items-center gap-3 text-blue-200">
            <div className="flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
              <span>{isHindi ? 'स्रोत: राष्ट्रीय शैक्षिक अनुसंधान और प्रशिक्षण परिषद (NCERT)' : 'Source: National Council of Educational Research and Training (NCERT)'}</span>
            </div>
            <a
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
          ...
          <nav className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-4">
            ...
            <Link to="/study-materials" className="hover:text-[#6C63FF] transition">
              {t('nav.studyMaterials')}
            </Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-slate-900 font-bold">{t('nav.ncertBooks')}</span>
            {selectedClass && (
              <>
                <ChevronRight className="w-3.5 h-3.5" />
                <span className="text-[#6C63FF] font-bold">{isHindi ? `कक्षा ${selectedClass}` : `Class ${selectedClass}`}</span>
              </>
            )}
            {selectedSubject !== 'ALL' && (
              <>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="max-w-2xl">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 text-blue-700 border border-blue-100 text-xs font-bold mb-3">
                ...
                <span>{isHindi ? 'कक्षा 1 से 12 • समस्त विषय • हिन्दी एवं अंग्रेजी माध्यम' : 'Classes 1 to 12 • All Subjects • Hindi & English Mediums'}</span>
              </div>
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-900 tracking-tight leading-tight">
                {t('study.ncertSectionTitle')}
              </h1>
            ...
            <div className="w-full lg:max-w-md">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder={isHindi ? 'खोजें: Science Class 10, गणित, Physics...' : 'Search: Science Class 10, Maths, Physics...'}
                  value={searchQuery}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500 font-medium">
              {t('study.showingBooksFor')} <strong>{isHindi ? `कक्षा ${selectedClass}` : `Class ${selectedClass}`}</strong>
            </span>
          </div>
            ...
            {Array.from({ length: 12 }, (_, i) => i + 1).map((cls) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold shrink-0 transition flex items-center justify-center min-w-[76px] ${
                    isSelected
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 flex-1">
            <span className="text-xs font-bold text-slate-500 shrink-0 uppercase">{t('study.subject')}:</span>
            <button
            ...
            >
              {t('study.allSubjects')} ({pagination.total})
            </button>
                  ...
                  <div className="relative bg-gradient-to-br from-slate-100 via-blue-50/50 to-indigo-50/30 p-5 flex items-center justify-center border-b border-slate-100 min-h-[200px]">
                    ...
                    <div className="w-28 h-36 rounded-lg shadow-md overflow-hidden bg-white border border-slate-200 group-hover:scale-105 transition-transform duration-300 flex items-center justify-center text-center p-2 relative">
                      ...
                      <div className="absolute inset-0 p-2 flex flex-col items-center justify-center bg-gradient-to-b from-blue-700 to-indigo-900 text-white text-center">
                        ...
                        <span className="text-[9px] text-blue-200 mt-0.5">{isHindi ? `कक्षा ${book.classNumber}` : `Class ${book.classNumber}`}</span>
                      </div>
                    </div>
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                      ...
                      <div className="flex items-center gap-2 mt-2 text-[11px] text-slate-500">
                        {book.chapterCount > 0 && (
                          <span className="flex items-center gap-1 font-semibold text-slate-700">
                            ...
                            <span>{book.chapterCount} {isHindi ? 'अध्याय' : 'Chapters'}</span>
                          </span>
                        )}
                        {book.bookCode && (
                          <>
                    ...
                    <div className="pt-3 border-t border-slate-100 flex items-center gap-2">
                      ...
                      >
                        ...
                        <span className="truncate">{t('study.officialPdf')}</span>
                      </button>
                    </div>
                  </div>
                </div>
